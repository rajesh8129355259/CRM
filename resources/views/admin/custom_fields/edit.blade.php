    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="mb-6">
                    <h2 class="text-2xl font-semibold text-gray-900">Edit Custom Field</h2>
                </div>
                ...
                <form action="{{ route('admin.custom-fields.update', $customField) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $customField->name) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="label" id="label" value="{{ old('label', $customField->label) }}"
                                class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                required>
                            @error('label')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <select name="type" id="type"
                                ...
                                <option value="select" {{ old('type', $customField->type) === 'select' ? 'selected' : '' }}>Select</option>
                        ...
                        <div id="options-container" style="display: none;">
                            ...
                            <textarea name="options" id="options" rows="3"
                                class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('options', is_array($customField->options) ? implode("\n", $customField->options) : '') }}</textarea>
                            @error('options')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <div class="flex items-center">
                                <input type="checkbox" name="is_active" id="is_active" value="1"
                                    class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                                    {{ old('is_active', $customField->is_active) ? 'checked' : '' }}>
